...
def main():
    ...
    optimizer = build_optimizer(
        model,
        lr=r_config["optimizer"]["lr"],
        weight_decay=r_config["optimizer"]["weight_decay"],
    )
    
    scheduler = build_scheduler(
        optimizer,
        mode="min",
        factor=r_config["scheduler"]["factor"],
        patience=r_config["scheduler"]["patience"],
    )
    
    early_stopper = EarlyStopping(
        patience=r_config["early_stopping"]["patience"],
        save_path="best_resnet.pth",
    )
    
    num_epochs = r_config["num_epochs"]
